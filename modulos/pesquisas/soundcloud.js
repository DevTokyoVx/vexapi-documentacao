...
const url = `https://vexapi.com.br/api/search/soundcloud?apikey=${apikey}&query=${encodeURIComponent(query)}`;
...
(async () => {
    try {
        console.log(`🚀 Pesquisando no SoundCloud: "${query}"...\n`);
        const dados = await buscarVexAPI(url);

        if (dados?.results?.length) {
            console.log('==============================');
            console.log('☁️ RESULTADOS DO SOUNDCLOUD');
            console.log('==============================\n');
            const primeiro = dados.results[0];
            console.log(`Título:    ${primeiro.title}`);
            console.log(`Artista:   ${primeiro.artist_name || 'N/A'}`);
            console.log(`Duração:   ${primeiro.time} segundos`);
