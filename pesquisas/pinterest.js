 ...
 *
 * Observações importantes sobre a Vex API Pinterest Imagens:
...
// URL completa do endpoint Pinterest
const url = `https://vexapi.com.br/api/pesquisa/pinterest?apikey=${apikey}&query=${encodeURIComponent(query)}`;
...
(async () => {
    try {
        ...
        if (dados?.results?.length > 0) {
            console.log(`✅ Total de resultados encontrados: ${dados.results.length}`);

            dados.results.forEach((item, i) => {
                console.log(`\n📌 Resultado ${i + 1}:`);
                console.log('Link da página: ', item.link);
                console.log('Link direto da imagem: ', item.directLink);
            });
