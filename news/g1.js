...
/**
 * Função que faz a requisição para a Vex API de notícias G1
 *
 ...
 *      - desc: descrição resumida (pode estar vazio)
 *      - categoria: categoria da notícia
 *      - link: link para a notícia completa
 *      - postado: quando foi postado
 *
 * Exemplo de URL:
 *  https://vexapi.com.br/api/noticias/g1?apikey=SEU_APIKEY&query=null
 *
...
const url = `https://vexapi.com.br/api/noticias/g1?apikey=${apikey}&query=${query}`;
...
(async () => {
    try {
        ...
        if (dados?.resposta?.length > 0) {
            console.log('✅ Últimas notícias do G1:');
            dados.resposta.forEach((noticia, i) => {
                console.log(`\n📰 Notícia ${i + 1}:`);
                console.log('Título: ', noticia.noticia);
                ...
                console.log('Postado: ', noticia.postado);
            });
        } else {
        ...
        console.error('❌ Erro ao buscar notícias do G1:', err.message);
    }
})();